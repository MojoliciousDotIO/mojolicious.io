...
We made one of the simplest Mojolicious apps possible to run the Minion dashboard for us, which we then link to from our own dashboard:
```
#!/usr/bin/env perl
...
my $env     = MyJob::EnvCheck->new->env_is;
my $config  = MyJob::Config->new->config;
my $minions = MyJob::Config->new({ filename => "environments/minions.yml" })->config;

plugin Minion => { mysql => MyJob::DBConnectionManager->new->get_connection_uri({ 
    db_type => 'feeds', 
    io_type => 'rw',
})};

plugin 'Minion::Admin' => { 
    return_to => $minions->{ dashboards }->{ $env },
    route     => app->routes->any('/'),
};

app->config( hypnotoad => { 
    listen   => [ 'http://*:' . $minions->{ dashboard_port } ],
    pid_file => $config->{ file_locations }->{ logs } . '/job_queue_dashboard.pid',
});
app->start;
```
